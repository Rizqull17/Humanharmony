...
`;
  ...
  onValue(ref(db, `users/` + userId + `/ujiAkhir`), (snapshot) => {
    if (snapshot.val().selesai) {
      onValue(ref(db, `users/` + userId), (snapshot) => {
        ...
        jawabanBenar.innerHTML = `${data.ujiAkhir.benar}`;
        jawabanSalah.innerHTML = `${data.ujiAkhir.salah}`;
        const nilai = document.getElementById("nilai");
        nilai.innerHTML = `${data.ujiAkhir.skor}`;
        const skor = `${data.ujiAkhir.skor}`;
        if (skor >= kkm) {
          const materinext = document.getElementById("beranda");
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    for (let j = 0; j < labelPilihanGanda.length; j++) {
      labelPilihanGanda[j].addEventListener("click", () => {
        ...
        set(ref(db, `users/` + userId + `/ujiAkhir/${i}`), {
          jawaban: pilihan,
        });
      });
    }
  ...
  onValue(ref(db, "Soal/evaluasi"), (snapshot) => {
    ...
    onValue(ref(db, `users/` + userId + `/ujiAkhir`), (jawabanSiswa) => {
      ...
      for (let i = 1; i <= 20; i++) {
        if (jawabanSiswaVal[i] != null) {
          if (
            ujiVal[`evaluasi-${i}`]["jawaban-benar"] ==
      ...
      // }
      update(ref(db, `users/` + userId + `/ujiAkhir`), {
        skor: benar * 5,
        benar: benar,
        salah: 20 - benar,
      });
