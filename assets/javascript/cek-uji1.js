...
onValue(ref(db, "Soal"), (snapshot) => {
  ...
  <div class="div-soal ${tampilanSoal}">
    ...
    <div class="card-body text-soal">${data[`Soal Uji1`][`Soal Uji1-${i}`]["text-soal"]
      }</div>
    </div>
    ...
    <label for="${i}a" class="label-${i}  " >
      ...
      <span class="ms-3 pilihan-jawaban pilihan-jawaban-${i}">a. ${data[`Soal Uji1`][`Soal Uji1-${i}`].a
      }</span>
    </label><br>
    <label for="${i}b" class="label-${i}  ">
      <input class="pilihan pilihan-${i}" type="radio" id="${i}b" name="no${i}" value="b" />
      <span class="ms-3 pilihan-jawaban pilihan-jawaban-${i}">b. ${data[`Soal Uji1`][`Soal Uji1-${i}`].b
      }</span>
    </label><br>
    <label for="${i}c" class="label-${i}  ">
      <input class="pilihan pilihan-${i}" type="radio" id="${i}c" name="no${i}" value="c" />
      <span class="ms-3 pilihan-jawaban pilihan-jawaban-${i}">c. ${data[`Soal Uji1`][`Soal Uji1-${i}`].c
      }</span>
    </label><br>
    <label for="${i}d" class="label-${i}  ">
      <input class="pilihan pilihan-${i}" type="radio" id="${i}d" name="no${i}" value="d" />
      <span class="ms-3 pilihan-jawaban pilihan-jawaban-${i}">d. ${data[`Soal Uji1`][`Soal Uji1-${i}`].d
      }</span>
    </label>
    </div>
  </div>
`;
  ...
  onValue(ref(db, `users/` + userId + `/uji1`), (snapshot) => {
    if (snapshot.val().selesai) {
      onValue(ref(db, `users/` + userId), (snapshot) => {
        ...
        const uji1 = document.getElementById("uji1");
        uji1.hidden = true;
        const hasilUji1 = document.getElementById("hasilUji1");
        hasilUji1.hidden = false;
        ...
        jawabanBenar.innerHTML = `${data.uji1.benar}`;
        jawabanSalah.innerHTML = `${data.uji1.salah}`;
        const nilai = document.getElementById("nilai");
        nilai.innerHTML = `${data.uji1.skor}`;
        const skor = `${data.uji1.skor}`;
        if (skor >= kkm) {
          const materinext = document.getElementById("materinext2_11");
          materinext.hidden = false;
        } else {
          const materinext = document.getElementById("materinext2_11");
          materinext.hidden = true;
        }
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    for (let j = 0; j < labelPilihanGanda.length; j++) {
      labelPilihanGanda[j].addEventListener("click", () => {
        ...
        set(ref(db, `users/` + userId + `/uji1/${i}`), {
          jawaban: pilihan,
        });
      });
    }
  ...
  onValue(ref(db, "Soal/Soal Uji1"), (snapshot) => {
    const ujiVal = snapshot.val();

    // get(child(dbRef, `users/`+userId+`/uji1`)).then ((jawabanSiswa) => {
    onValue(ref(db, `users/` + userId + `/uji1`), (jawabanSiswa) => {
      ...
      for (let i = 1; i <= 10; i++) {
        if (jawabanSiswaVal[i] != null) {
          if (
            ujiVal[`Soal Uji1-${i}`]["jawaban-benar"] ==
