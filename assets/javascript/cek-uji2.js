...
onValue(ref(db, "Soal"), (snapshot) => {
  ...
  const kkm = snapshot.val().kkm.uji2;
  console.log(kkm);
  ...
  <div class="div-soal ${tampilanSoal}">
    ...
    <div class="card-body text-soal">${data[`Soal Uji2`][`Soal Uji2-${i}`]["text-soal"]
      }</div>
    </div>
    ...
    <label for="${i}a" class="label-${i}  " >
      ...
      <span class="ms-3 pilihan-jawaban pilihan-jawaban-${i}">a. ${data[`Soal Uji2`][`Soal Uji2-${i}`].a
      }</span>
    </label><br>
    <label for="${i}b" class="label-${i}  ">
      <input class="pilihan pilihan-${i}" type="radio" id="${i}b" name="no${i}" value="b" />
      <span class="ms-3 pilihan-jawaban pilihan-jawaban-${i}">b. ${data[`Soal Uji2`][`Soal Uji2-${i}`].b
      }</span>
    </label><br>
    <label for="${i}c" class="label-${i}  ">
      <input class="pilihan pilihan-${i}" type="radio" id="${i}c" name="no${i}" value="c" />
      <span class="ms-3 pilihan-jawaban pilihan-jawaban-${i}">c. ${data[`Soal Uji2`][`Soal Uji2-${i}`].c
      }</span>
    </label><br>
    <label for="${i}d" class="label-${i}  ">
      <input class="pilihan pilihan-${i}" type="radio" id="${i}d" name="no${i}" value="d" />
      <span class="ms-3 pilihan-jawaban pilihan-jawaban-${i}">d. ${data[`Soal Uji2`][`Soal Uji2-${i}`].d
      }</span>
    </label>
    </div>
  </div>
`;
  ...
  onValue(ref(db, `users/` + userId + `/uji2`), (snapshot) => {
    if (snapshot.val().selesai) {
      onValue(ref(db, `users/` + userId), (snapshot) => {
        ...
        const uji2 = document.getElementById("uji2");
        uji2.hidden = true;
        const hasilUji2 = document.getElementById("hasilUji2");
        hasilUji2.hidden = false;
        ...
        jawabanBenar.innerHTML = `${data.uji2.benar}`;
        jawabanSalah.innerHTML = `${data.uji2.salah}`;
        const nilai = document.getElementById("nilai");
        nilai.innerHTML = `${data.uji2.skor}`;
        const skor = `${data.uji2.skor}`;
        if (skor >= kkm) {
          const materinext2_1 = document.getElementById("materinext3_11");
          materinext2_1.hidden = false;
        } else {
          const materinext2_1 = document.getElementById("materinext3_11");
          materinext2_1.hidden = true;
        }
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    for (let j = 0; j < labelPilihanGanda.length; j++) {
      labelPilihanGanda[j].addEventListener("click", () => {
        ...
        set(ref(db, `users/` + userId + `/uji2/${i}`), {
          jawaban: pilihan,
        });
      });
    }
  ...
  onValue(ref(db, "Soal/Soal Uji2"), (snapshot) => {
    const ujiVal = snapshot.val();

    // get(child(dbRef, `users/`+userId+`/uji2`)).then ((jawabanSiswa) => {
    onValue(ref(db, `users/` + userId + `/uji2`), (jawabanSiswa) => {
      ...
      for (let i = 1; i <= 10; i++) {
        if (jawabanSiswaVal[i] != null) {
          if (
            ujiVal[`Soal Uji2-${i}`]["jawaban-benar"] ==
      ...
      // }
      update(ref(db, `users/` + userId + `/uji2`), {
        skor: benar * 10,
        benar: benar,
        salah: 10 - benar,
      });
      ...
      if (result.isConfirmed) {
        if (sudahDijawab.length < 10) {
          ...
          onValue(ref(db, `users/` + userId), (snapshot) => {
            ...
            const skor = `${data.uji2.skor}`;
            if (skor >= kkm) {
              const materinext2_1 = document.getElementById("materinext2_1");
              materinext2_1.hidden = false;
            } else {
              const materinext2_1 = document.getElementById("materinext2_1");
              materinext2_1.hidden = true;
